...
static char *stat_filename_base = "energy_count_rank_";
static FILE *out_file;
static char stat_filename_computed[1024];
...
static void write_line(tn_energy *energy_data) {
  double const distance =
      compute_network_distance(energy_data->my_core, energy_data->dest_core);
...
void save_energy_stats(tn_energy *energy_data, int source_rank) {
  ...
  if (is_file_init == false) {
    ...
    sprintf(stat_filename_computed, "%s%i.csv", stat_filename_base,
            source_rank);
