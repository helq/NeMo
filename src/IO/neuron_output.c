...
static FILE *outNetworkCfgFile;
static FILE *outNeuronCfgFile;

static list_t networkStructureList;

static rqueue_t *nc_data_q;
static rqueue_t *is_working;
static pthread_t filethread;

static int maxNetworkListSize = 1024;
  ...
  do {
    ...
    if (rqueue_isempty(nc_data_q) == 0) {
      ...
      for (int ax = 0; ax < NEURONS_IN_CORE; ax++) {
        ...
        // fprintf(outNeuronCfgFile, ",%i",  n->synapticWeight[n->axonTypes[ax]]
        // && n->synapticConnectivity[ax]);
...
void saveNetworkStructureMPI() {
  ...
  if (done == 0) {
    MPI_File net_file;
    MPI_File_open(MPI_COMM_WORLD, network_mpi_out_filename,
                  MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &net_file);
    ...
    long entry_size =
        sizeof(char) * 16; // each entry will contain 16 chars. Will include the
                           // ',' chars (so a total of 15 digits).
    // core, neuronID, DC, DA, axon_conn_list, axon_types, weights, weight_mode,
    // newline
    long size_of_params = (4 + AXONS_IN_CORE + AXONS_IN_CORE +
                           NUM_NEURON_WEIGHTS + NUM_NEURON_WEIGHTS + 1) *
                          entry_size;
    // we need to save a line for each neuron:
    long long total_write_size =
        CORES_IN_SIM * NEURONS_IN_CORE * size_of_params;
    long rank_write_size = total_write_size / 1;
    // and the offset is the total write size / num_neurons_per_ranks (since the
    // write size is neuron-based)
    ...
    for (int i = neuron_start; i < num_neurons_per_rank; i++) {
      ...
      for (int j = 0; j < AXONS_IN_CORE; j++) {
        sprintf(neuron_data, "%s%15d,", neuron_data,
                n->synapticConnectivity[i]);
      }
      for (int j = 0; j < AXONS_IN_CORE; j++) {
    ...
    MPI_File_write_at_all(net_file, offset, neuron_data, rank_write_size,
...
void debug_neuron_connections(tn_neuron_state *n, tw_lp *lp) {
  ...
  fprintf(rank_output, "%li,%li,%li,%li,%li,%li", lp->gid, n->myCoreID,
          n->myLocalID, n->outputCoreDest, n->outputNeuronDest, n->outputGID);
  for (int i = 0; i < NUM_NEURON_WEIGHTS; i++) {
...
/**
 * non-threaded version of the saveNeuronNetworkStructure() function. Rather
 * than taking a single neuron's state and using a thread to write out the data,
 * this function saves the neuron state through one large loop.
 */
void saveNetworkStructure() {
  ...
  for (int core = 0; core < CORES_IN_SIM / 1; core++) {
    for (int neuron = 0; neuron < NEURONS_IN_CORE; neuron++) {
      ...
      for (int ax = 0; ax < NEURONS_IN_CORE; ax++) {
        sprintf(lntxt, "%s,%i", lntxt,
                n->synapticWeight[n->axonTypes[ax]] &&
                    n->synapticConnectivity[ax]);
        fprintf(outNeuronCfgFile, "%s,%i", lntxt,
                n->synapticWeight[n->axonTypes[ax]] &&
                    n->synapticConnectivity[ax]);
        fflush(outNeuronCfgFile);
      }
