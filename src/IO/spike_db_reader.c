...
static uint64_t combine(unsigned int high, unsigned int low) {
...
//}
//
//static long execCountCallback(void *nu, int argc, char **argv, char **azColName) {
//
//  printf("%s = %s\n", azColName[0], argv[0] ? argv[0] : "NULL");
//  return strtol(argv[0], NULL, 10);
//}
//
//static int spikeQueryCallback(void *splist, int argc, char **argv, char **colName) {
...
//}
//
//static char *genSelectQuery(id_type axon, id_type core) {
//  char *query = calloc(sizeof(char), 1024);
//  sprintf(query, "%s %lli %s %lli %s", q1, axon, q2, core, q3);
//  return query;
//}
//
//static char *genCountQuery(id_type axon, id_type core) {
//  char *query = calloc(sizeof(char), 1024);
 ...
 *
 */
static void assembleCoreQuery(int type, id_type core) {
...
static int doesCoreHaveSpikesDB(id_type core) {
...
static uint64_t interleave(uint32_t time, uint32_t axid) {
 ...
 */
static int getSpikesSynapseDB(void *timeList, id_type core) {
  assembleCoreQuery(1, core);
  errmsg = calloc(sizeof(char), 1024);
  sqlite3_stmt *res;
 ...
 */
static int getSpikesFromAxonSQL(void *M, id_type coreID, id_type axonID) {
...
static int connectToDB(char *filename) {
  int st = -999;
  if (spikedb_isopen == 0) {
...
static int closeDB(char *filename) {
  int st = 0;
  if (spikedb_isopen == 1) {
