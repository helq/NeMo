...
const long double WALL_OFFSET =
    0.0000002; //! Some sort of offset for the wall clock time - what is a good
               //! value?
const long double CPU_OFFSET = 0.0000002; //! The CPU time for recv. messages.
const long double NEURO_CORE_CLOCK =
    1000; //! Neuromorphic core speed (cycles / second).
 ...
 * @return
 */
uint64_t chipToRank(uint64_t chipID) { return chipID; }

uint64_t coreToChip(uint64_t coreID) { return coreID / CORES_IN_CHIP; }
...
int getCurrentTick(double now) { return (int)now; }
...
char *generateMsg(uint64_t sourceChip, uint64_t destChip, double twTimeSend,
                  char *type, unsigned long dumpID) {

  char *outStr =
      calloc(sizeof(char), 1024); // alloc new string - using this instead of
  ...
  sprintf(outStr, "%li,%s %llu %llu %.21Lf %.21Lf %.17Lf %.17Lf %i %i %i %i\n",
          t, type, sourceChip, destChip, wallStart, wallEnd, cpuStart, cpuEnd,
          COUNT, DTYPE, COMM, TAG);

  return outStr;
}
char *generateIsend(uint64_t sourceChip, uint64_t destChip, double twTimeSend,
                    unsigned long dumpID) {

  return generateMsg(sourceChip, destChip, twTimeSend, "MPI_Isend", dumpID);
}
char *generateIrecv(uint64_t sourceChip, uint64_t destChip, double twTimeSend,
                    unsigned long dumpID) {
  return generateMsg(sourceChip, destChip, twTimeSend + (3 * CPU_OFFSET),
                     "MPI_Irecv", dumpID);
}
...
void saveSendMessage(unsigned long long sourceCore, unsigned long long destCore,
                     double twTime, unsigned long dumpID, FILE *outputFile) {
  uint64_t sourceChip = coreToRank(sourceCore);
  uint64_t destChip = coreToRank(destCore);
  char *isend = generateIsend(sourceChip, destChip, twTime, dumpID);
  fprintf(outputFile, "%s", isend);
  free(isend);
}
void saveRecvMessage(unsigned long long sourceCore, unsigned long long destCore,
                     double twTime, unsigned long dumpID, FILE *outputFile) {
  long sourceChip = coreToRank(sourceCore);
  long destChip = coreToRank(destCore);
